...
itemList = {"Bread", "Dairy", "Meat", "Fruit", "Vegetables"}
...
def main():
    ...
    search_all_stores(itemList)
    time.sleep(60)
    driver.quit()

def search_multiple_products(store_url, product_keyword_list):
    driver.switch_to.new_window('tab')
    driver.get(store_url)

    for item in product_keyword_list:
...
def search_all_stores(product_keywords):
    for store in Stores:
        ...
        search_multiple_products(store.value, product_keywords)
